...
export class RegisterContianer extends React.Component<RegisterContianerProps, RegisterContianerState> {
    state = { registerProcess: RegisterProcess.Ready}

    doRegister = (email: string, password: string) => {
        this.setState({registerProcess: RegisterProcess.Loading})

        getMessagingClient().post<RegisterRequest, RegisterResponse>('api/', { Email: email, Password: password })
            .then(response => {
                this.setState({
